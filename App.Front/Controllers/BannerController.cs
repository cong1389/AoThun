...
namespace App.Front.Controllers
{
	public class BannerController : FrontBaseController
	{
		...
		{
			IEnumerable<Banner> banners = this._bannerService.FindBy((Banner x) => x.PageId == 7 && x.PageBanner.Position == 5 && x.Status == 1 && (!x.FromDate.HasValue || DbFunctions.DiffHours((TimeSpan?)x.ToDate.Value, (TimeSpan?)DateTimeOffset.UtcNow.Offset) >= (int?)0) && (!x.ToDate.HasValue || DbFunctions.DiffHours((TimeSpan?)x.ToDate.Value, (TimeSpan?)DateTimeOffset.UtcNow.Offset) <= (int?)0), false);
			return base.PartialView(banners);
		}
	}
}